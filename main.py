...
def save_processed(processed):
    """Saves the processed items list to processed.json."""
...
def main():
    ...
    for feed in config.get('feeds', []):
        ...
        for entry in parsed_feed.entries:
            ...
            if not item_id or item_id in processed:
                continue
            ...
            try:
                ...
                script = generate_script(
                    ...
                    article_text,
                    config['generation'].get('target_length_minutes', 5)
                )
                ...
                with TemporaryDirectory() as temp_dir:
                    audio_files = generate_audio_for_lines(
                        openai_client, lines,
                        config['audio']['openai_tts_model'],
                        config['audio']['host_a_voice'],
                        config['audio']['host_b_voice'],
                        temp_dir
                    )
                    ...
                    if stitch_audio(audio_files, stitched_local_path, tags=mp3_tags):
                        ...
                        if upload_to_drive(drive_service, stitched_local_path, config['google_drive']['folder_id'], final_mp3_name):
                            generate_podcast_rss(drive_service, config)
                        
                # 5. Finalize
                processed[item_id] = True
